...
;var ah=x().toString();function bh(a){O.call(this,1,arguments)}
...
c=r("yt.timing.reportbuilder_")){if(c=c(d,a,e,void 0))rh(c),sh()}else{var f=G("TIMING_ACTION",void 0),b=G("CSI_SERVICE_NAME","youtube");c={v:2,s:b,action:f};if(S.now&&S.timing){var h=S.timing.navigationStart+S.now(),h=Math.round(x()-h);e.yt_hrd=h}var h=G("TIMING_INFO",{}),k;for(k in h)e[k]=h[k];k=e.srt;delete e.srt;var l;void 0===d.srt&&(k||0===k||(l=S.timing||{},k=Math.max(0,l.responseStart-l.navigationStart),isNaN(k)&&e.pt&&(k=e.pt)),k||0===k)&&(e.srt=Math.round(k));e.h5jse&&(h=window.location.protocol+
r("ytplayer.config.assets.js"),(h=S.getEntriesByName?S.getEntriesByName(h)[0]:null)?e.h5jse=Math.round(e.h5jse-h.responseEnd):delete e.h5jse);d.aft=qh();h=d._start;if("cold"==e.yt_lt){l||(l=S.timing||{});var n;a:if(n=l,n.msFirstPaint)n=Math.max(0,n.msFirstPaint);else{var u=window.chrome;if(u&&(u=u.loadTimes,ha(u))){var u=u(),ba=1E3*Math.min(u.requestTime||Infinity,u.startLoadTime||Infinity),ba=Infinity===ba?0:n.navigationStart-ba;n=Math.max(0,Math.round(1E3*u.firstPaintTime+ba)||0);break a}n=0}0<
n&&n>h&&(d.fpt=n);n=a||ph().span;u=l.redirectEnd-l.redirectStart;0<u&&(n.rtime_=u);u=l.domainLookupEnd-l.domainLookupStart;0<u&&(n.dns_=u);u=l.connectEnd-l.connectStart;0<u&&(n.tcp_=u);u=l.connectEnd-l.secureConnectionStart;l.secureConnectionStart>=l.navigationStart&&0<u&&(n.stcp_=u);u=l.responseStart-l.requestStart;0<u&&(n.req_=u);u=l.responseEnd-l.responseStart;0<u&&(n.rcv_=u);S.getEntriesByType&&th(d);(l=G("SPEEDINDEX_FOR_ACTIONS",void 0))&&-1<l.indexOf(f)&&(l=hc(),f=dh(),n=hc(),0<f.qb&&(e.si=
f.qb,d.vsc=uh(f.Jc),d.sics=l,d.sice=n));l=[];if(document.querySelector&&S&&S.getEntriesByName)for(var R in kh)if(f=document.querySelector(R))n="",u=f.nodeName,"SCRIPT"==u?n=f.src:"LINK"==u&&(n=f.href),n&&(f=S.getEntriesByName(n))&&f[0]&&l.push(kh[R]+"."+Math.round(f[0].duration));l.length&&(e.rs=l.join(","))}f=oh();R=f.pbr;l=f.vc;f=f.pbs;R&&l&&f&&R<l&&l<f&&1==ph().info.yt_vis&&"youtube"==b&&(ph().info.yt_lt="hot_bg",R=d.vc,l=d.pbs,delete d.aft,e.aft=Math.round(l-R));(R=G("PREVIOUS_ACTION"))&&(e.pa=
R);e.p=G("CLIENT_PROTOCOL")||"unknown";e.t=G("CLIENT_TRANSPORT")||"unknown";window.navigator&&window.navigator.sendBeacon&&(e.ba=1);for(var Ab in e)"_"!=Ab.charAt(0)&&(c[Ab]=e[Ab]);d.ps=hc();Ab={};R=[];for(var pc in d)"_"!=pc.charAt(0)&&(n=Math.round(d[pc]-h),dg("enable_negative_ticks")||(n=Math.max(n,0)),Ab[pc]=n,R.push(pc+"."+n));c.rt=R.join(",");pc=c;R=[];for(var Gb in a)"_"!=Gb.charAt(0)&&R.push(Gb+"."+Math.round(a[Gb]));pc.it=R.join(",");(Gb=r("ytdebug.logTiming"))&&Gb(c,Ab,a);sh();G("EXP_DEFER_CSI_PING")?
...
;function Hk(a,b){V.call(this);this.i=b;for(var c=mj("yt-remote-online-screen-ids")||"",c=c?c.split(","):[],d={},e=this.i(),f=0,h=e.length;f<h;++f){var k=e[f].id;d[k]=A(c,k)}this.b=d;this.j=a;this.g=this.h=NaN;this.f=null;Ik("Initialized with "+L(this.b))}
...
g.update=function(){Ik("Updating availability on schedule.");var a=this.i(),b=Za(this.b,function(b,d){return b&&!!gk(a,d)},this);
Kk(this,b)};
...
g.Rb=function(){I(this.g);this.g=NaN;this.f&&this.f.abort();var a=Nk(this);if($a(a)){var b=ak(this.j,"/pairing/get_screen_availability");this.f=bk(this.j,b,{lounge_token:db(a).join(",")},w(this.Pd,this,a),w(this.Od,this))}else Kk(this,{}),Jk(this)};
...
function Nk(a){var b={};z(a.i(),function(a){a.token?b[a.token]=a.id:this.H("Requesting availability of screen w/o lounge token.")});
return b}
function Mk(a){var b=db(Za(a.b,function(a){return a}));
b.sort(Xa);b.length?U("yt-remote-online-screen-ids",b.join(","),60):nj("yt-remote-online-screen-ids");a=Ja(a.i(),function(a){return!!this.b[a.id]},a);
U("yt-remote-online-screens",Ka(a,Tj),60)}
